    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {doctors.slice(0,10).map((item,index)=>(
          <div onClick={()=>navigate(`/cita/${item._id}`)} className='border border-green-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
      ...
      <button className='bg-blue-50 text-gray-600 px-12 py-3 rounded-full mt-10'>Más</button>
    </div>
  )
}
